...
| `P6-07 Implement exact selected GGUF quant tensor-slice semantics` | Inventory every per-tensor `ggml_type` used by the pinned recipe and replace toy semantics with exact model-relevant reference fixtures. | In Progress: CPU `Q4_K` reference only; artifact inventory remains pending #80. |
