...
| Fixed free-RAM gates disable safe development or process-bounded counters overstate system headroom | Critical | 0/6-9 | T-101 uses separate workload-relative physical/commit payloads, authoritative `GetPerformanceInfo` system commit counters, checked arithmetic, two explicit lanes, and a non-promotable development receipt | #109 supplies the pure policy, authoritative Windows sampling, raw counter-source manifest fields, and deterministic 8/12/15 GiB plus commit-pressure tests | #103 must serialize the full admission receipt/token and integrate live watchdog cancellation and fault evidence before C2; lane thresholds remain provisional under registry change control |
...
| Sustained GPU work runs without a hardware supervisor | Critical | 5-9 | single-owner GPU lease, bounded child process, wall-clock deadline, cancellation, thermal/power/VRAM/host-commit watchdog, and fail-closed artifact state | #103 remains open and incomplete with provisional work in draft PR #112; `AGENTS.md` prohibits sustained or full-model CUDA work until it closes | #81/#82 prerequisites, #103 implementation and fault-injection tests |
