...
| Compute research | A tiny CPU Q4 decode/GEMV correctness scaffold exists. It is not a model-scale kernel or performance claim. |
...
- CUDA support is never an implicit setup step. The not-yet-established CUDA
  kernel lane must not be invoked or presented as available.
...
After a CPU build, a bounded probe can emit lifecycle telemetry and a manifest:
