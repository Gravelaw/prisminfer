...
- Every pull request, including documentation, tests, dependencies, workflows, follow-ups, and packet integration, requires a separate non-authoring reviewer invocation before merge. Invoke an independent agent run or qualified human for that PR after its reviewable head is ready. The reviewer inspects the current PR patch, applicable contracts, validation evidence, and claim boundary; the author cannot substitute a self-review. Record findings and their disposition on the PR.
- Safety-critical changes require an independent functional/safety review by a non-authoring agent run, preferably a higher-assurance reasoning/pro or coding-specialized preset, or a qualified human. The reviewer must inspect the exact patch and retained executable/fault evidence; the author cannot mark the issue complete, and model opinion alone is insufficient. A dedicated security scan is not required for ordinary packet completion before the final program gate at #102.
...
- Post each PR's independent review receipt on that PR before merge, including findings and their disposition. A checkpoint or parent receipt may cover unchanged code, but each PR still gets its own separate reviewer invocation to inspect integration and uncovered changes. After a head, base, contract, or evidence change, the reviewer must assess the affected delta and update the receipt before merge.
...
- Before merging any PR, verify its independent review receipt covers the current head/tree and base, all blocking findings are resolved, and required checks pass. Safety-critical code additionally receives independent exact-head functional/safety review. Security-diff, repository-wide, and deep security scans are deferred until the final program gate at #102; they are not routine PR or packet-completion gates before then.
...
## Library-Completion Security Review Policy

- Until the final program gate at #102, prioritize functional execution, deterministic correctness, integration, applicable testing, and completion of dependency-safe PRs. Dedicated security reviews and Codex Security scans are optional unless the user explicitly requests one or an early-trigger condition below applies.
- The deferral does not waive the basic safety floor: checked arithmetic, bounded inputs and loops, native process/Job containment, controlled executable and artifact identity, fail-closed admission/telemetry, exclusive lease ownership, bounded timeout/cancellation/cleanup, simulation and negative/fault tests, and host/code-integrity protections remain mandatory where applicable.
- A focused functional/safety reviewer may reject a T2 change for a concrete containment, resource-bound, cleanup, telemetry, data-integrity, or machine-stability defect. Fixing those defects is functionality completion, not a deferred security exercise.
- Security-relevant findings discovered during implementation may be recorded and fixed in their natural dependency-safe packet. Noncritical deferred findings do not block unrelated functionality PRs merely because a consolidated scan has not run; they must retain an owner, disposition, and target gate and cannot be silently discarded.
- Trigger a dedicated security review before #102 only for a credible critical vulnerability, new public or untrusted-user exposure, secrets or privileged network/service integration, a material sandbox/authorization boundary change beyond the existing contained worker contract, or explicit user direction. A credible critical vulnerability immediately blocks the affected PR, packet, promotion, and hardware/model dispatch until it is fixed or disproven with retained evidence.
- #89 performs the usable-runtime functional, safety, evidence, and claim audit without requiring Codex Security. After #96 and #101 complete the implementation program and its applicable test/evidence matrix, #102 performs the first mandatory consolidated product/repository security review and the final release/publication evidence and claim audit.
...
- **T0 documentation/tracker:** run changed-file Markdown/link/schema checks, Plan/Project synchronization when applicable, and concise whitespace validation. Every T0 PR receives an independent, risk-appropriate PR review; governing safety or claim contracts receive T2 review.
- **T1 ordinary CPU code:** run the smallest deterministic build/test slice at each checkpoint and the complete applicable CPU/hosted lane once at the packet head. Every T1 PR receives an independent PR review; a packet-level receipt may cover unchanged checkpoints, but cannot replace the PR-specific reviewer invocation.
- **T2 safety-critical code or governance:** admission/allocation arithmetic, untrusted parsers, native worker/supervisor, recovery, WDDM/DXGI evidence, workflows, provider boundaries, CUDA, and safety-contract changes require focused negative/fault tests plus independent exact-head functional/safety review before merge. A dedicated security-diff scan is deferred under the library-completion policy unless an early-trigger condition applies.
...
- Use Codex Security skills when the user requests them, an early-trigger condition in the library-completion policy applies, or the program reaches the #102 consolidated review gate. Do not start or require a security-diff, repository, or deep scan as a routine pre-#102 PR merge gate.
