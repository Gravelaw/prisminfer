...
- Safety-critical changes require an independent functional/safety review by a non-authoring agent run, preferably a higher-assurance reasoning/pro or coding-specialized preset, or a qualified human. The reviewer must inspect the exact patch and retained executable/fault evidence; the author cannot mark the issue complete, and model opinion alone is insufficient. A dedicated security scan is not required for ordinary packet completion before the usable-runtime gate defined in `Plan.md`.
...
- Safety-critical code still receives independent exact-head functional/safety review before merge. Security-diff, repository-wide, and deep security scans are deferred until the usable-runtime and final-release gates declared by `Plan.md`; they are not routine PR or packet-completion gates before then.
...
## Development-First Security Review Policy

- Until the usable-runtime gate at #89, prioritize functional execution, deterministic correctness, integration, and completion of dependency-safe PRs. Dedicated security reviews and Codex Security scans are optional unless the user explicitly requests one or an early-trigger condition below applies.
...
- Trigger a dedicated security review before #89 only for a credible critical vulnerability, new public or untrusted-user exposure, secrets or privileged network/service integration, a material sandbox/authorization boundary change beyond the existing contained worker contract, or explicit user direction.
- After #87 and #88 establish a usable selector and contained deterministic replay path, #89 performs the first consolidated risk/security review against the working product. #102 retains the final release/publication security, evidence, and claim review.
...
- Use Codex Security skills when the user requests them, an early-trigger condition in the development-first policy applies, or the program reaches the #89/#102 consolidated review gates. Do not start or require a security-diff, repository, or deep scan as a routine pre-#89 PR merge gate.
